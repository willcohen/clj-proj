;; A copy of the PROJ fn-defs data in net.willcohen.proj.fndefs. A clj-kondo
;; hook runs in SCI and cannot read a var out of the project it lints.
;;
;; Do not edit the generated region by hand. Run `bb kondo:fndefs` after any
;; edit to src/cljc/net/willcohen/proj/fndefs.cljc. The helpers below the end
;; marker are hand-written.
(ns hooks.fn-defs)
...
(defn is-c-context-fn?
  ...
  (let [arg-specs (:argtypes fn-def)]
    (cond
      ...
      ;; These destroy fns take a context but are not context-managed.
...
(defn generate-docstring
  ...
  (let [c-name-str (name fn-key)
    ...
    (str "Calls the native `" c-name-str "` function.\n\n"
         ...
         (if (seq arg-lines) (clojure.string/join "\n" arg-lines) "  (None)")
         "\n\n"
         return-desc)))